...
class ProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cvCardView.setOnClickListener {
            ...
            val etEditDate = dialogLayout.findViewById<EditText>(R.id.etEditDate)
            val etEditMemo = dialogLayout.findViewById<EditText>(R.id.etEditMemo)

            with(builder) {
                setTitle("Edit Memo Kamu")
                setPositiveButton("UPDATE") { dialog, which ->
                    ...
                    GlobalScope.launch {
                        val result = db.memoDao().updateItem(memo)

                        runOnUiThread {
                            if (result > 0) {
                                Toast.makeText(
                                    applicationContext,
                                    "Sukses merubah data ${memo.isiMemo}",
                                    Toast.LENGTH_LONG
                                ).show()
                            } else {
                                Toast.makeText(
                                    applicationContext,
                                    "Gagal merubah data ${memo.isiMemo}",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
                ...
                setPositiveButton("DELETE") { dialog, which ->
                    Log.d("Main", "delete button pressed")
                    GlobalScope.launch {
                        val result = db.memoDao().deleteItem(memo)

                        runOnUiThread {
                            if (result > 0) {
                                Toast.makeText(
                                    applicationContext,
                                    "Sukses menghapus memo ${memo.isiMemo}",
                                    Toast.LENGTH_LONG
                                ).show()
                            } else {
                                Toast.makeText(
                                    applicationContext,
                                    "gagal menghapus memo ${memo.isiMemo}",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
                ...
                setNegativeButton("CANCEL") { dialog, which ->
                    Log.d("Main", "cancel button pressed")
                }
                setView(dialogLayout)
                show()
            }
